...
import { X, CheckCircle2, Zap, Users, Plus, Check } from 'lucide-react';
...
export const CourseDetailModal: React.FC<CourseDetailModalProps> = ({
    ...
    useEffect(() => {
        if (isOpen && course) {
            trackViewContent({ content_name: course.title, value: course.price, currency: country.currencyCode });
        }
    }, [isOpen, course, country]);

    const handleCartToggle = () => {
        if (!course) return;
        if (!isInCart) {
            trackAddToCart({ content_name: course.title, value: course.price, currency: country.currencyCode });
        }
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            {/* Modal Card */}
            <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col z-10 animate-[scaleUp_0.25s_ease-out]">
                <style>{`
          @keyframes scaleUp {
        ...
        `}</style>
                ...
                {/* Hero Image + Software Badge + Close Button */}
                ...
                <div className="p-5 border-t border-gray-100 bg-white shrink-0">
                    <div className="flex items-center justify-between">
                        <div className="flex items-baseline gap-2">
                            <span className="text-3xl font-display font-bold text-gray-900">{country.currencySymbol}{course.price.toLocaleString()}</span>
                            <span className="text-sm text-gray-400 line-through">{country.currencySymbol}{course.originalPrice.toLocaleString()}</span>
